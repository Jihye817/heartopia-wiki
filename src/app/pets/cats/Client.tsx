...
const CAT_ACCENT = "#c96a42";
const CAT_BG = "var(--wiki-cat-pets-bg)";
const CAT_BORDER = "#f5d0c4";

// ── Subcomponents ─────────────────────────────────────────────────────────────

function CatCard({ cat }: { cat: Pet }) {
  return (
    <Link
      href={`/pets/cats/details/${cat.id}`}
      ...
      onMouseEnter={(e) => {
        e.currentTarget.style.borderColor = `${CAT_ACCENT}66`;
      }}
      onMouseLeave={(e) => {
      ...
      >
        {cat.thumbnail ? (
          <div className="relative h-full w-full">
            <Image src={cat.thumbnail} alt="" fill className="object-contain" />
          </div>
        ) : (
...
function CatListView({ cats }: { cats: Pet[] }) {
  return (
...
export default function CatsClient({ cats }: CatsClientProps) {
